...
namespace ConsoleApplication2
{
    internal class Program
    {
        ...
        public static void Main(string[] args)
        {
            ...
            UsingBuffer();
            stopwatch.Stop();
            Console.WriteLine("Safe took {0} milliseconds", stopwatch.ElapsedTicks);
            
            
            stopwatch.Restart();
            UsingUnsafe1();
            stopwatch.Stop();
            Console.WriteLine("Unsafe1 took {0} milliseconds", stopwatch.ElapsedTicks);
            
            stopwatch.Restart();
            UsingUnsafe2();
            stopwatch.Stop();
            Console.WriteLine("Unsafe2 took {0} milliseconds", stopwatch.ElapsedTicks);

        }
    }
}s_result = mapplus2Class.mapPlus2(xs)
    }
}
\end{minted}
...
By calling entry_main(), we first call ReadStrArray<int> to parse an integers
...
\begin{figure}[H]
  \centering
\begin{minted}[linenos]{csharp}
(int, byte[], int) futhark_main(int byte_array_length, byte[] byte_array, 
                                int byte_array_elms)
{
...
The full program is available in figure \ref{fig:memoryperformancebenchmark} in the
