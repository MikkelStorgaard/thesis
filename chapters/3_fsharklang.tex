...
in pseudocode in figure \ref{fig:pseudofusion}.
Similarly, in an OpenCL program, the short code example is translated into a
single kernel, as shown in figure \ref{fig:pseudokernel}. Notice here that the global
thread id has a similar function to the iterator variable i in the non-OpenCL
for-loop.




\subsubsection{Aggressive SOAC fusioning}

returns copies of the functions
 better soacs through mutable functions, however we will still miss out on
 performance gains stemming from Futhark SOAC fusion.
